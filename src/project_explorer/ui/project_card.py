...
class ProjectCard(QWidget):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.nav_bar.reload_button.clicked.connect(
            lambda: self.set_project(load_project(self.project.path))
        )
    ...
    def set_project(self, project: Project | InvalidProject | MissingProject) -> None:
    ...
    def _update_project(self) -> None:
        ...
        if isinstance(self.project, InvalidProject):
            self.nav_bar.error_label.setToolTip(
                "Project found but contains no meta data"
            )
            self.nav_bar.error_label.show()
        else:
    ...
    def event(self, event: QEvent) -> bool:
        if event.type() == ImageLoadedEvent.s_type:
            ...
            self.bg.add_image(QPixmap.fromImage(image_loaded_event.image))
    ...
    def _edit_screen(self) -> None:
        ...
        my_progress_dialog.setWindowTitle(
            f"{action} {project_name} ({self.project.path.as_posix()})"
        )
    ...
    def _save_project(self, name: str, tags: list[str]) -> None:
        ...
        with open(
            self.project.path / "project-info.json", "w", encoding="utf-8"
        ) as file:
